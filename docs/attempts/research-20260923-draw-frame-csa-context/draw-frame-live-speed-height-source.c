...
p_im_one_path:
        /* Original +0.2 and -0.2 reset predecessors each read frame[0] and
         * compute oy before setting p_im=1; route around the later common read. */
        if (ply[player_id]->sx >= 0.0) {
            if (ply[player_id]->sx < 0.2) {
                ...
                oy = 1 - customFrame->h;
                p_im = 1;
                goto p_im_one_oy_ready;
            }
        } else if (ply[player_id]->sx > -0.2) {
            ply[player_id]->frame = 0;
            customFrame = custom.frame[0];
            oy = 1 - customFrame->h;
            p_im = 1;
    ...
    /* 2605/2606: common frame-zero height computation for the other original
     * predecessors; status-zero's narrow-speed route enters edge handling above. */
    customFrame = custom.frame[0];
    oy = 1 - customFrame->h;
p_im_one_oy_ready:
