...
`getFloorData` has the historical 107-byte extent and fully matches its
...
are all equal.
`add_floor` now has a 608-byte source reconstruction. It shifts the 31 prior
...
table. Its historical body is also 608 bytes and remains `DIFFER` at the
`floor_shrink` guard register allocation, so it receives no exact credit. In the
