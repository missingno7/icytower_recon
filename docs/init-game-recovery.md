...
`allegro_init()` itself is nonfatal in the oracle. The long graphics-mode
guidance instead belongs to the later null-screen failure branch, which logs
the null screen before switching to text mode.
...
High-score allocation now reports its distinct text-mode failure. A missing
...
Joystick installation now enables `ctrl.use_joy` and retrieves the gamepad
mapping only inside the file/default mapping branch. The initializer also
records `window` for a successful direct windowed request; direct fullscreen
and fullscreen-fallback paths retain the fullscreen state used by cursor and
display-switch setup.

The primary datafile stage now decrypts the original mutable six-byte password
through `pwd_garble_string` before use, clears it after loading, and reuses
