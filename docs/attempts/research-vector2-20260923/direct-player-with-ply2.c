...
void handle_player_collision_vector_2(int lastX, int lastY)
{
    ...
    current_x = (int)ply[player_id]->x;
    current_y = (int)ply[player_id]->y + 1;
    plx2 = lastX - 11;
    ...
    floor_x1 = 0;
    floor_x2 = 0;
    getFloorData(&map, (int)ply[player_id]->y, &floor_y, &floor_x1, &floor_x2);
    if (floor_y == -12345678) {
        getFloorData(&map, ply2, &floor_y, &floor_x1, &floor_x2);
        if (floor_y == -12345678) {
            floor_y = 0;
            floor_x1 = 0;
            floor_x2 = 0;
    ...
    if (debug) {
        if (key[KEY_F2]) {
            ...
            line(screen, plx1, current_y, plx2, ply2, col2);
            line(screen, prx1, current_y, prx2, ply2, col2);
        }
    }
    left = line_intersect(floor_x1, floor_y, floor_x2, floor_y,
        plx1, current_y, plx2, ply2, &left_x, &left_y);
    right = line_intersect(floor_x1, floor_y, floor_x2, floor_y,
        prx1, current_y, prx2, ply2, &right_x, &right_y);
    if (!left && !right) {
