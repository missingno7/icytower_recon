...
create_replay is behaviorally reconstructed. It allocates the 0x8ac-byte replay object, initializes its fixed header, default name/date, score/floor/combo state, and replay-event storage. The fixed `ITR140` header is a file-local mutable data object, restoring the historical pair of data loads into the six-byte destination and improving the candidate from 242 to 250 bytes. The remaining four-byte gap is the allocation-failure branch layout, so the historical 254-byte body remains `DIFFER`.
