...
void draw_frame(BITMAP *bmp)
{
    ...
    if (ply[player_id]->status) {                       /* 2589 */
        /* Historical code keeps status 3 and status 2 as separate predecessor
         * blocks with distinct `sy` comparison sequences (original offsets
         * 1819..1852 and 2556..2591). Keep the status tests independent here
         * to test whether the merged else-if shape caused GCC to share the
         * downstream frame-zero height path. Status values 0..3 are exclusive. */
        ...
        if (ply[player_id]->status == 2) {                /* 2591 */
            if (ply[player_id]->sy > 3.0)
                p_im = 7;
            else
                p_im = 6;
        }
        if (ply[player_id]->status == 1) {                /* 2592 */
            if (ply[player_id]->sy < -3.0)
                p_im = 5;
            else
                p_im = 6;
...
p_im_one_path:
        /* Original positive and negative speed exits enter distinct frame-reset
         * blocks before their respective frame-zero height loads. Keep those
         * predecessor forms separate while retaining the independent status arms. */
        if (ply[player_id]->sx >= 0.0) {
            if (ply[player_id]->sx < 0.2)
                ply[player_id]->frame = 0;
        } else {
            if (ply[player_id]->sx > -0.2)
                ply[player_id]->frame = 0;
