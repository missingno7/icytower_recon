...
candidate is 1,656 bytes against the oracle's 1,938 bytes. The recovered
setup now initializes the menu parameter assets, its embedded `Tcontrol`, and
the typed main menu. It also handles the `-check` replay/ad path and the first-run
