...
embedded `Tcontrol` through the recovered `DATAFILE` records, handles the
`-check` replay/ad route, and implements the first-run guest-profile
transition. The isolated `-O2` candidate is 1,656 bytes, so this is
behavioral/source recovery rather than an exact match. Exceptional exit handling and the remaining
