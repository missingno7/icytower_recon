...
new=new.replace('void fldads_update_local_adimg(const char *pRemoteName);\n','').replace('void *fldads_threadmain(void *data);\n','')
(outdir/'start-after-threadmain-no-forward-decls.c').write_text(new,encoding='cp1252')
out,ref,build,r=compile_overlay(target,src,new,'fldads-threadmain-20260924-start-after-no-forward-decls',dumps=False,headers=headers)
if r.returncode: raise SystemExit(r.stderr)
report=compare(out/'unit.o',ref['historical_cu'],ROOT/'assets/icytower15.exe',OBJDUMP)
(outdir/'start-after-threadmain-no-forward-decls-comparison.json').write_text(json.dumps(report,indent=2))
fs={f['name']:f for f in report['functions']}
summary={'order':order,'candidate_source_sha256':hashlib.sha256(new.encode('cp1252')).hexdigest(),'object_sha256':hashlib.sha256((out/'unit.o').read_bytes()).hexdigest(),'function_matches':report.get('function_matches'),'whole_text_contribution_equal':report.get('whole_text_contribution_equal'),'object_match':report.get('object_match'),'cu_match':report.get('cu_match'),'target':{k:fs['fldads_threadmain'].get(k) for k in ['status','candidate_size','original_size','first_difference']},'statuses':{n:f['status'] for n,f in fs.items()},'build_output':str(out.relative_to(ROOT))}
(outdir/'start-after-threadmain-no-forward-decls-summary.json').write_text(json.dumps(summary,indent=2)); print(json.dumps(summary,indent=2))
