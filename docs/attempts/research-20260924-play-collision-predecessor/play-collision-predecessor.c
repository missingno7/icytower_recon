    ...
    while (playing && !closeButtonClicked) {   /* 3536 (3534 tests playing; both tests and both loop
        ...
        if (!itrcheck) {                                                /* 3706 */
            ...
            if (old_map_pos % 16 > map.offset % 16                        /* 3783 */
                || scroll_acc > 15) {                                      /* 3787 */
                add_floor(&map);                                           /* 3789 */
                if ((unsigned)collision_type > 4u)
                    goto play_collision_type_error;
            } else if ((unsigned)collision_type > 4u) {
                goto play_collision_type_error;
            }
        }
        ...
        goto play_collision_type_done;

play_collision_type_error:
        allegro_message("unknown collision type");                      /* 3826 */
play_collision_type_done:
                ...
                if (is_playing_custom_game) {
                    ...
                    if (gotHigh) {                                                       /* 4749 */
                        ...
                        if (isGuest) {
                            ...
                            strcpy(summary_scroller_message, hints[new_rand() % 45]);    /* 4770 */
                        }
                    } else {
                        if (isGuest) {
                            strcpy(summary_scroller_message,
