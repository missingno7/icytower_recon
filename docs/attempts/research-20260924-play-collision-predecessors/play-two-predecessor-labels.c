...
int play(void)
{
    ...
    while (playing && !closeButtonClicked) {   /* 3536 (3534 tests playing; both tests and both loop
        ...
        if (!itrcheck) {                                                /* 3706 */
             ...
             * order below). */
            if (old_map_pos % 16 > map.offset % 16 || scroll_acc > 15) {
                add_floor(&map);
                goto after_floor_check;
            }
        }
        goto no_floor_check;
after_floor_check:
        if ((unsigned)collision_type > 4)
            goto unknown_collision_type;
        goto collision_dispatch;
no_floor_check:
        if ((unsigned)collision_type > 4)
            goto unknown_collision_type;
        goto collision_dispatch;
unknown_collision_type:
        ...
        goto collision_type_done;
collision_dispatch:
        lastY = level;
        switch (collision_type) {
        case 3: handle_player_collision_original(midX, lastY); break;
        case 2: handle_player_collision_old(midX, lastY); break;
        case 1: handle_player_collision_vector(midX, lastY); break;
        case 0: handle_player_collision_vector_2(midX, lastY); break;
        case 4: handle_player_collision_combo(midX, lastY); break;
        default: goto unknown_collision_type;
        }
        goto collision_type_done;
collision_type_done:
