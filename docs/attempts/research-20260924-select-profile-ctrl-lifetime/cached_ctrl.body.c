                                ...
                                int numProfiles, Tprofile_control *ctrl)
{
    ...
    while (!closeButtonClicked && !done) {
        cycle_count = 0;
        checkMenuFocus();
        poll_control(cached_ctrl, 1);
        if (is_any(cached_ctrl) && !ctrl_wait) {
            if (is_down(cached_ctrl))
                simulate_keypress(0x5500);
            else if (is_up(cached_ctrl))
                simulate_keypress(0x5400);
            else if (is_fire(cached_ctrl))
