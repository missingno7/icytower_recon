void draw_menu(BITMAP *bmp, Tmenu *m, Tmenu_params *mp, int cx, int y, int dx)
{
    ...
    do {
        ...
        if (m->flags & 1)
            draw_sprite(bmp, mp->bullet,
                        x - 3 - (mp->bullet)->w, y - 3);
        textout_ex(bmp, (FONT *)mp->font, str, x, y, -1, -1);
        if (m->flags & 16) {
            draw_sprite(bmp, (BITMAP *)mp->data[mp->fo + 0].dat, x + 215, y + 10);
            draw_sprite(bmp, (BITMAP *)mp->data[mp->fo + 1].dat, x + 236, y + 10);
            draw_sprite(bmp, (BITMAP *)mp->data[mp->fo + 2].dat, x + 252, y + 10);
        }
        if (m->flags & 32) {
            BITMAP *b;
