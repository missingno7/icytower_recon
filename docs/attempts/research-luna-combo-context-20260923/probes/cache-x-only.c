...
void handle_player_collision_combo(int lastX, int lastY)
{
    ...
    solid1 = is_solid(&map, (int)cached_x - 11, (int)p->y);
    solid2 = is_solid(&map, (int)cached_x + 11, (int)p->y);
    any11 = solid1;
    any12 = solid2;
    any23 = 0;
    any22 = 0;
    ...
    if (debug) {
        if (key[KEY_F2]) {
            ...
            line(screen, (int)cached_x - 11, (int)p->y + 1, lastX - 11, lastY, col2);
            line(screen, (int)cached_x + 11, (int)p->y + 1, lastX + 11, lastY, col2);
        }
    }
    left = line_intersect(fx1, fy1, fx2, fy1,
        (int)cached_x - 11, (int)p->y + 1, lastX - 11, lastY, &ilx, &ily);
    right = line_intersect(fx1, fy1, fx2, fy1,
        (int)cached_x + 11, (int)p->y + 1, lastX + 11, lastY, &irx, &iry);
    if (!left && !right) {
        p->edge = 0;
        return;
    }
