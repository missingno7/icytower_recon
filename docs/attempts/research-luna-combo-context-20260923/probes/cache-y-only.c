...
void handle_player_collision_combo(int lastX, int lastY)
{
    ...
    solid1 = is_solid(&map, (int)p->x - 11, (int)cached_y);
    solid2 = is_solid(&map, (int)p->x + 11, (int)cached_y);
    any11 = solid1;
    any12 = solid2;
    any23 = 0;
    any22 = 0;
    ...
    if (debug) {
        if (key[KEY_F2]) {
            ...
            line(screen, (int)p->x - 11, (int)cached_y + 1, lastX - 11, lastY, col2);
            line(screen, (int)p->x + 11, (int)cached_y + 1, lastX + 11, lastY, col2);
        }
    }
    left = line_intersect(fx1, fy1, fx2, fy1,
        (int)p->x - 11, (int)cached_y + 1, lastX - 11, lastY, &ilx, &ily);
    right = line_intersect(fx1, fy1, fx2, fy1,
        (int)p->x + 11, (int)cached_y + 1, lastX + 11, lastY, &irx, &iry);
    if (!left && !right) {
        p->edge = 0;
        return;
    }
